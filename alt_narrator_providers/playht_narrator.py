...
async def async_play_audio(data):
    ...
    buff_size = calculate_buffer_size(
    ...
    )  # Add a 1/10 of it to be sure it finishes
...
async def async_main(from_error=False, text=None, debug_camera=False):
    ...
    print(
        "This alt narrator provider is still in development. Please use the main narrator.py file instead."
    )
    return
    ...
    while count != max_times:
        ...
        try:
            ...
            await async_play_audio(
                clientPlayHT.tts(text, voice_engine="PlayHT2.0-turbo", options=options)
            )
